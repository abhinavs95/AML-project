...
def factorize(S, num_factors, R=None, lambda_reg=1e-5, num_iterations=20, init_std=0.01, verbose=False, dtype='float32', recompute_factors=recompute_factors, *args, **kwargs):
    """
    ...
    for i in xrange(num_iterations):
        ...
        U = recompute_factors(V, S, lambda_reg=lambda_reg, dtype=dtype, *args, **kwargs)

        if verbose:
            print "    time since start: %.3f seconds" % (time.time() - start_time)
            print "    recompute item factors V"

        V = recompute_factors(U, ST, lambda_reg=lambda_reg, dtype=dtype, *args, **kwargs)
